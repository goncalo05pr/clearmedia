...
CREATE POLICY "formations_select_policy" ON public.formations FOR SELECT USING (is_active = true);
CREATE POLICY "formations_admin_policy" ON public.formations FOR ALL USING (EXISTS (SELECT 1 FROM auth.users WHERE auth.users.id = auth.uid() AND auth.users.raw_user_meta_data->>'role' = 'admin'));

CREATE POLICY "chapters_select_policy" ON public.formation_chapters FOR SELECT USING (EXISTS (SELECT 1 FROM public.formations f WHERE f.id = formation_chapters.formation_id AND f.is_active = true));
CREATE POLICY "chapters_admin_policy" ON public.formation_chapters FOR ALL USING (EXISTS (SELECT 1 FROM auth.users WHERE auth.users.id = auth.uid() AND auth.users.raw_user_meta_data->>'role' = 'admin'));

CREATE POLICY "modules_select_policy" ON public.formation_modules FOR SELECT USING (EXISTS (SELECT 1 FROM public.formation_chapters fc JOIN public.formations f ON f.id = fc.formation_id WHERE fc.id = formation_modules.chapter_id AND f.is_active = true));
CREATE POLICY "modules_admin_policy" ON public.formation_modules FOR ALL USING (EXISTS (SELECT 1 FROM auth.users WHERE auth.users.id = auth.uid() AND auth.users.raw_user_meta_data->>'role' = 'admin'));

CREATE POLICY "purchases_user_policy" ON public.user_purchases FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "purchases_admin_policy" ON public.user_purchases FOR ALL USING (EXISTS (SELECT 1 FROM auth.users WHERE auth.users.id = auth.uid() AND auth.users.raw_user_meta_data->>'role' = 'admin'));

CREATE POLICY "profiles_user_policy" ON public.profiles FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "profiles_admin_policy" ON public.profiles FOR ALL USING (EXISTS (SELECT 1 FROM auth.users WHERE auth.users.id = auth.uid() AND auth.users.raw_user_meta_data->>'role' = 'admin'));

CREATE POLICY "reviews_select_policy" ON public.reviews FOR SELECT USING (is_public = true);
CREATE POLICY "reviews_user_policy" ON public.reviews FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "reviews_admin_policy" ON public.reviews FOR ALL USING (EXISTS (SELECT 1 FROM auth.users WHERE auth.users.id = auth.uid() AND auth.users.raw_user_meta_data->>'role' = 'admin'));

CREATE POLICY "cms_content_select_policy" ON public.cms_content FOR SELECT USING (true);
CREATE POLICY "cms_content_update_policy" ON public.cms_content FOR UPDATE USING (EXISTS (SELECT 1 FROM auth.users WHERE auth.users.id = auth.uid() AND auth.users.raw_user_meta_data->>'role' = 'admin'));
CREATE POLICY "cms_content_insert_policy" ON public.cms_content FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM auth.users WHERE auth.users.id = auth.uid() AND auth.users.raw_user_meta_data->>'role' = 'admin'));
...
CREATE TRIGGER handle_formations_updated_at BEFORE UPDATE ON public.formations FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();
CREATE TRIGGER handle_chapters_updated_at BEFORE UPDATE ON public.formation_chapters FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();
CREATE TRIGGER handle_modules_updated_at BEFORE UPDATE ON public.formation_modules FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();
CREATE TRIGGER handle_purchases_updated_at BEFORE UPDATE ON public.user_purchases FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();
...
CREATE TRIGGER handle_cms_content_updated_at BEFORE UPDATE ON public.cms_content FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();
...
SELECT f.id, 'Chapitre 1: Introduction', 1 FROM public.formations f WHERE f.title = 'Social Ads Mastery'
UNION ALL
SELECT f.id, 'Chapitre 2: Les bases', 2 FROM public.formations f WHERE f.title = 'Social Ads Mastery'
UNION ALL
SELECT f.id, 'Chapitre 1: Démarrage', 1 FROM public.formations f WHERE f.title = 'Funnel Premium';

INSERT INTO public.formation_modules (chapter_id, title, type, content_url, "order", is_free)
SELECT fc.id, 'Introduction au cours', 'video', 'https://www.youtube.com/watch?v=dQw4w9WgXcQ', 1, true
...
UNION ALL
SELECT fc.id, 'Premiers pas', 'text', '{"content": "Bienvenue dans ce module..."}', 2, false
FROM public.formation_chapters fc 
JOIN public.formations f ON f.id = fc.formation_id 
WHERE f.title = 'Social Ads Mastery' AND fc.title = 'Chapitre 1: Introduction';
